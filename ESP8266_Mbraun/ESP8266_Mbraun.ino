...
Adafruit_BME280 bme;
bool bme_status = false; // Track if BME init failed
...
void sendDataToServer() {
  ...
  // Check if sensor was found during setup
  if (!bme_status) {
    logMessage("%s\n", "BME sensor missing. Cannot read.");
    failedRequests++;
    return;
  }

  // --- CHANGE 1: WAKE UP SENSOR ---
  // Command the sensor to take one reading now.
...
void setup() {
  ...
  // --- CHANGE 2: INITIALIZE BME HERE ---
  // We do this once in setup rather than in the loop
  bme_status = bme.begin(); // Try 0x76 first, change to 0x77 if needed
  if (!bme_status) {
      logMessage("BME280 sensor not found!");
  } else {
      logMessage("BME280 Found. Configuring Forced Mode.");
      
      // --- CHANGE 3: CONFIGURE FORCED MODE ---
